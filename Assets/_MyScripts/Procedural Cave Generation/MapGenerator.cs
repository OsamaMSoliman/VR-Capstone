	...
	private void GenerateMap()
	{
		...
		for ( int x = 0 ; x < borderedMap.GetLength(0) ; x++ )
			for ( int y = 0 ; y < borderedMap.GetLength(1) ; y++ )
				borderedMap[x , y] = (x >= borderSize && x < width + borderSize && y >= borderSize && y < height + borderSize) ? map[x - borderSize , y - borderSize] : 1;

		meshGen = new MeshGenerator(gameObject , walls , cave , wallHeight , is2D);
		meshGen.GenerateMesh(borderedMap , 1);

		// assign proper scale for the ground
		transform.GetChild(0).localScale = new Vector3(width / 10.0f , 1 , height / 10.0f);
		// make the ground height Zero globably (push it down by the wall height)
		transform.GetChild(0).localPosition = Vector3.down * wallHeight;
		// center the Maze by moving it up by the wall height
